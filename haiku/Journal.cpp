...
Journal::_CheckRunArray(const run_array *array)
{
	...
	if (array->MaxRuns() != maxRuns
		...
		|| array->CountRuns() <= 0) {
		printf("run count: %d, array max: %d, max runs: %d\n",
...
//TODO: fix this line, but we're not journalling for now so who cares
			ssize_t written = blockSize/*write_pos(fVolume->Device(),
...
// TODO!!! convert following line to windows stuff
	...
	cache_end_transaction(fVolume->BlockCache(), fTransactionID, _blockNotify, (void*)logEntry);
...
//TODO: who cares about timestamps for now
