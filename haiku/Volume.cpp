...
{
	printf("opening device: %s, bfs partition %i\n",device, partitionNr);
...
//	DISK_GEOMETRY *pdg;
	...
	*/
	...
	RtlInitUnicodeString(&UnicodeFilespec, L"\\Device\\HardDisk0\\Partition3"); // bfs partition?
...
//	DISK_GEOMETRY *pdg;
	...
	//RtlInitUnicodeString(&UnicodeFilespec, L"\\Device\\HardDisk0\\Partition3"); // bfs partition?
	RtlInitUnicodeString(&UnicodeFilespec, wsDisk); // bfs partition?
	InitializeObjectAttributes(&ObjectAttributes,           // ptr to structure
                               &UnicodeFilespec,            // ptr to file spec
                               OBJ_CASE_INSENSITIVE,        // attributes
                               NULL,                        // root directory handle
...
{
	// partitionNr x is the x'th bfs partition, starting at 0 for the first partition
	bfsNr = partitionNr;
	...
	uint32 flags)
{
	...
	DeviceOpener opener(device, bfsNr, O_RDWR);
