...
char*
GetLastErrorText(DWORD nErrorCode)
{
	char* msg;
...
//	HANDLE hDevice; // handle to the drive to be examined
	...
	HANDLE hDevice = CreateFile(drive, NULL, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, NULL, NULL);
	...
	while (hDevice != INVALID_HANDLE_VALUE) {
		...
		index++;
		sprintf(drive, PHYSICALDRIVE, index);
		hDevice = CreateFile(drive, NULL, FILE_SHARE_READ | FILE_SHARE_WRITE, NULL, OPEN_EXISTING, NULL, NULL);	
	}
...
int 
listDir(HWND hWnd, HTREEITEM* node, TV_ITEM* tvi, ofstream* debug, vector<Volume>* volumes)
{
	void* iter;
	dirent d;
	int children = 0;
...
int
addChild(HWND hWnd, HTREEITEM* parent, char* t1, int icon, TreeData* td, std::ofstream* debug)
{
...
int
listPartitions(int disk, HWND h, HTREEITEM* node, TV_INSERTSTRUCT* s, std::ofstream* debug)
{	
